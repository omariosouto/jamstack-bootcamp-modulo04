...
import { render, waitFor, screen } from '../../../infra/test/testUtils';
...
describe('<FormLogin />', () => {
  describe('when form fields are valid', () => {
    test('complete the submission', async () => {
      ...
      await waitFor(() => expect(inputUsuario.value).toBe('someusername'));

      const inputSenha = screen.getByPlaceholderText('Senha');
      user.type(inputSenha, 'somepass');
      await waitFor(() => expect(inputSenha.value).toBe('somepass'));
